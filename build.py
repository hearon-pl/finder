...
GEND = re.compile(r"(wtyk|gniazdo)(\s+kątow[ya])?\s*$", re.I)
NOISE = (
...
def parse_end(v, k):
    ...
    if k == "speakon":
        g = "F" if re.search(r"gniazdo|żeńsk", s, re.I) else ("M" if re.search(r"\bwtyk\b|męsk", s, re.I) else None)
        return {"type": "Speakon", "gender": g, "label": "Speakon"}
    if k == "hdmi":
        g = "F" if re.search(r"gniazdo|żeńsk", s, re.I) else ("M" if re.search(r"wtyk|męsk", s, re.I) else None)
        return {"type": "HDMI", "gender": g, "label": ("Gniazdo HDMI" if g == "F" else "Wtyk HDMI")}
    ...
    gender = None
    if gm:
        gender = "M" if gm.group(1).lower() == "wtyk" else "F"
    ...
    if re.search(r"mini", t, re.I) and re.search(r"xlr", t, re.I):
        t = "mini XLR 3-pin"
    gw = "wtyk" if gender == "M" else ("gniazdo" if gender == "F" else "")
...
def build_data(recs):
    ...
    for x in recs:
        ...
        g = groups.setdefault(key, {
            ...
            "sig": x["sig"], "lengths": [], "variants": [], "cats": set(),
        })
        if x["len"] is not None and x["len"] not in g["lengths"]:
            g["lengths"].append(x["len"])
        g["variants"].append({"len": x["len"], "price": x["price"], "url": x["url"]})
        g["cats"].update(x["cats"])

    for g in groups.values():
        g["lengths"].sort()
        g["variants"].sort(key=lambda v: (v["len"] or 0))
        prices = [v["price"] for v in g["variants"] if v["price"]]
        g["price_min"] = min(prices) if prices else None
        g["price_max"] = max(prices) if prices else None
        g["url"] = g["variants"][0]["url"] if g["variants"] else ""
    ...
    keep = ("name", "brand", "aType", "aGender", "aLabel", "bType", "bGender",
            "bLabel", "sig", "lengths", "price_min", "price_max", "url", "cats")
    return {
