...
# wartość opisująca cały kabel: dwie rodziny złączy rozdzielone myślnikiem
RODZINY_RE = r"xlr|jack|rca|cinch|speakon|powercon|hdmi|rj45|banan"
PODEJRZANE = re.compile(rf"({RODZINY_RE}).*\s[-–]\s.*({RODZINY_RE})", re.I)
...
def kind(v):
    ...
    if not v:
        return None
    # opis całego kabla ("2x RCA (Cinch) - XLR Żeński") to nie złącze —
    # taka wartość trafia zwykle w pole "Typ", nie w "Złącze A/B"
    if PODEJRZANE.search(v):
        return None
    vl = v.lower().strip()
...
def parse_products(path):
    ...
    for event, elem in ET.iterparse(path, events=("end",)):
        ...
        # 1) jeśli produkt ma wypełnione Złącze A i Złącze B — używamy TYLKO ich
        # 2) w przeciwnym razie przeszukujemy blok parametrów (starsze produkty)
        za, zb = params.get("Złącze A", ""), params.get("Złącze B", "")
        if kind(za) and kind(zb):
            pairs = [(za, kind(za)), (zb, kind(zb))]
        else:
